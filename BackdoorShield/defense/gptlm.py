...
class GPT2LM:
    def __init__(self, use_tf=False, device=None, little=False):
        ...
        import transformers
        self.use_tf = use_tf
        self.tokenizer = transformers.GPT2TokenizerFast.from_pretrained("/root/bdad/models/gpt2")

        if use_tf:
            self.lm = transformers.TFGPT2LMHeadModel.from_pretrained("/root/bdad/models/gpt2")
        else:
            self.lm = transformers.GPT2LMHeadModel.from_pretrained("/root/bdad/models/gpt2", from_tf=False)
            self.lm.to(device)
