...
def evaluaion(loader):
    ...
    with torch.no_grad():
        for padded_text, attention_masks, labels in loader:
            ...
            output = model(padded_text, attention_mask=attention_masks)[0]
...
def transfer_bert():
    ...
    scheduler = transformers.get_linear_schedule_with_warmup(optimizer,
                                                             num_warmup_steps=0,
                                                             num_training_steps=transfer_epoch * len(train_loader_clean))
    best_acc = -1
    last_loss = 100000
    try:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--clean_data_path', required=True)
    parser.add_argument('--poison_data_path', required=True)
    ...
    # Load model
    model = BertForSequenceClassification.from_pretrained('/root/bdad/models/bert-base-uncased', num_labels=4 if data_selected == 'ag' else 2)
