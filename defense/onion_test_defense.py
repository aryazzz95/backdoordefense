...
def filter_sent(split_sent, pos):  # 剔除指定位置的词
    words_list = split_sent[: pos] + split_sent[pos + 1:]
    return ' '.join(words_list)


def evaluaion(loader):  # 评估准确性
    model.eval()
    total_number = 0
    total_correct = 0
    with torch.no_grad():
...
def get_processed_sent(flag_li, orig_sent):  # 重新构建只含标志词为1的词的句子
    sent = []
...
def get_processed_poison_data(all_PPL, data, bar):  # 去掉ppl小于bar的
    processed_data = []
...
if __name__ == '__main__':
    ...
    # 加载预训练的 Bert 模型并加载其权重
    model = BertForSequenceClassification.from_pretrained('/root/bdad/models/bert-base-uncased', num_labels=2)  # 或者num_labels=4等
    model.load_state_dict(torch.load(args.model_path))  # 加载保存的模型权重
    model = model.to('cuda' if torch.cuda.is_available() else 'cpu')  # 将模型转移到设备上（GPU或CPU）
